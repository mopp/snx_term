...
void vga_scroll(){
	*(VGA|SCROLL) = scroll_cnt;
	if(scroll_cnt == VGA_MAX_ROW) scroll_cnt = 0;
	else scroll_cnt++;
}

void inc_adress_y(){
	if(vga_current_y == VGA_MAX_ROW) vga_current_y = 0;
	else vga_current_y++;
...
void move_cursor(){
	if(scroll_flg == 0) *(VGA|CURSOR1) = vga_current;
	else *(VGA|CURSOR1) = 0x0E80 | vga_current_x;
}

void line_break(){
	if(vga_current_y == DISP_ROW_MAX) scroll_flg = 1;
	if(scroll_flg == 1){
